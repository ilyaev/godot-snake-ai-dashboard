...
var DQNAgent = function DQNAgent(env, opt) {
...
DQNAgent.prototype = {
  ...
  forwardQ: function forwardQ(net, s, needsBackprop) {
  ...
  learn: function learn(r1) {
    ...
    if (!(this.r0 == null) && this.alpha > 0) {
      ...
      for (var k = 0; k < this.learningStepsPerIteration; k++) {
        var ri = UTILS.randi(0, this.exp.length); // todo: priority sweeps?
        var e = this.exp[ri];
  ...
  // a1 is never used!
  learnFromTuple: function learnFromTuple(s0, a0, r0, s1, a1) {
    // want: Q(s,a) = r + gamma * max_a' Q(s',a')
    // compute the target Q value
    var tmat = this.forwardQ(this.net, s1, false);
    var qmax = r0 + this.gamma * tmat.w[UTILS.maxi(tmat.w)];
    // now predict
