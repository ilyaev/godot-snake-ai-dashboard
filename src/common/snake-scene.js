...
module.exports = {
    instance: function instance() {
        ...
        var calculateQvalue = function calculateQvalue() {
            ...
            for (var x = 0; x <= scene.maxX; x++) {
                for (var y = 0; y <= scene.maxY; y++) {
                    ...
                    s.setFrom([
                        x / scene.maxX,
                        y / scene.maxY,
                        (scene.target.x - x) / scene.maxX,
                        (scene.target.y - y) / scene.maxY
                    ]);
                    var amat = scene.agent.forwardQ(scene.agent.net, s, false);
                    var a = R.maxi(amat.w); // returns index of argmax action
                    scene.qvalues[x][y] = amat.w.map(function(one) {
                        return Math.round(one * 1000) / 1000;
                    });
                }
            }
        };
        ...
        var calculateAverage = function calculateAverage(period) {
            var res = scene.history.slice(-period).reduce(
